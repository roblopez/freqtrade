...
class SharedNew(IStrategy):
    ...
    # trailing_stop_positive = 0.005 for BTFD
    trailing_stop_positive = 0.01
    trailing_stop_positive_offset = 0.03
    #trailing_stop_positive_offset = 0.015 for BTFD
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe['tema9'] = ta.TEMA(dataframe, timeperiod=9)
    ...
    def populate_buy_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # if the candle was more than 5 minutes ago, don't buy anything
        current_time = (datetime.now(pytz.UTC) - timedelta(hours=1)).time().strftime('%H:%M:%S')
        candle_time = dataframe['date'].tail(1).iat[0].time().strftime('%H:%M:%S')
        diff_time = (datetime.strptime(current_time, '%H:%M:%S') - datetime.strptime(candle_time, '%H:%M:%S'))
        # self.logger.info(f'current_time = {current_time}, candle_time = {candle_time}, diff in time in seconds: {diff_time.seconds}')
        if diff_time.seconds > 300: # 5 minutes
            dataframe['buy'] = 0
            return dataframe

        ##########Breakouts Below Here##########
        dataframe.loc[ # Breakout: TEMA crosses over longer TEMA
            (
                (qtpylib.crossed_above(dataframe['tema50'], dataframe['tema21'])) &
                (dataframe['adx'] > 35)
            ),
            'buy'] = 1
        ...
        dataframe.loc[ # Breakout: TEMA crosses over longer TEMA
            (
                (qtpylib.crossed_above(dataframe['tema14'], dataframe['tema21'])) &
                (dataframe['adx'] > 35)
            ),
            'buy'] = 1
        ...
        dataframe.loc[ # Breakout: Increasing momentum and positive price action
            (
                (qtpylib.crossed_above(dataframe['adx'], 35)) &
                (dataframe['ema21'] > dataframe['ema50'])
            ),
            'buy'] = 1
        ##########Breakouts Above Here##########
        ...
        # if the candle was more than 2 minutes ago, don't buy anything
        #current_time = (datetime.now(pytz.UTC) - timedelta(hours=1)).time().strftime('%H:%M:%S')
        #candle_time = dataframe['date'].tail(1).iat[0].time().strftime('%H:%M:%S')
        #diff_time = (datetime.strptime(current_time, '%H:%M:%S') - datetime.strptime(candle_time, '%H:%M:%S'))
        ## self.logger.info(f'current_time = {current_time}, candle_time = {candle_time}, diff in time in seconds: {diff_time.seconds}')
        #if diff_time.seconds > 120: # 2 minutes
        #    dataframe['buy'] = 0
    ...
    def populate_sell_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe.loc[ # Breakout: TEMA crosses over longer TEMA
            (
                (qtpylib.crossed_below(dataframe['tema5'], dataframe['tema14']))
            ),
            'sell'] = 1

        dataframe.loc[ # Breakout: TEMA crosses over longer TEMA
            (
                (qtpylib.crossed_below(dataframe['tema14'], dataframe['tema21']))
            ),
            'sell'] = 1

        dataframe.loc[ # Breakout: TEMA crosses over longer TEMA
            (
                (qtpylib.crossed_below(dataframe['tema21'], dataframe['tema50']))
            ),
            'sell'] = 1

        dataframe.loc[
            (
		#(qtpylib.crossed_below(dataframe['mfi'], 95))  # Signal: MFI crosses below 95
            ),
            'sell'] = 1

        dataframe.loc[
            (
		#(qtpylib.crossed_below(dataframe['rsi'], 95))  # Signal: RSI crosses below 95
            ),
            'sell'] = 1

        dataframe.loc[
            (
                #(qtpylib.crossed_below(dataframe['close'], dataframe['bb_upperband'] * 1.02 ))
            ),
            'sell'] = 1

        dataframe.loc[
            (
		#(qtpylib.crossed_below(dataframe['cmo'], 85))  # Signal: RSI crosses below 95
            ),
            'sell'] = 1

        dataframe.loc[
            (
		#(qtpylib.crossed_below(dataframe['cci'], 350))  # Signal: RSI crosses below 95
            ),
            'sell'] = 1
